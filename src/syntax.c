	...
	else if (STRCMP(key, "CTERMFG") == 0 || STRCMP(key, "CTERMBG") == 0)
	{
	  if (!init || !(HL_TABLE()[idx].sg_set & SG_CTERM))
	  {
		...
		if (is_normal_group)
		{
		    ...
		    if (!gui.in_use && !gui.starting && !HI_COLOR)
#endif
		    {
			must_redraw = CLEAR;
...
# ifdef FEAT_GUI
		/* In GUI guifg colors are only used when recognized */
		i = color_name2handle(arg);
		if (i != INVALCOLOR || STRCMP(arg, "NONE") == 0 || !gui.in_use || !HI_COLOR)
		{
...
static name_to_rgb stupid_hardcoded_rgb_txt[] = {
{ "snow", 255, 250, 250 },
...
{ NULL }
    ...
    guicolor_T
stupid_rgb_color(name)
    char_u  *name;
{
    int i;
    long n;
    char hex[3];
    guicolor_T c = INVALCOLOR;


    for (i = 0; stupid_hardcoded_rgb_txt[i].name != NULL; i++)
    {
	name_to_rgb nrgb = stupid_hardcoded_rgb_txt[i];
	if (strcasecmp(name, nrgb.name) == 0)
	    return (nrgb.r << 16) + (nrgb.g << 8) + nrgb.b;
    }
    if (name[0] != '#')
	return c;
    for (i = 1; i < 7; i++)
	if (index("0123456789abcdefABCDEF", name[i]) == NULL)
	    return c;
    c = 0;
    for (i = 0; i < 3; i++)
    {
    ...
    char_u  *name;
{
    ...
    guicolor_T c = gui_get_color(name);
    if (HI_COLOR && c == INVALCOLOR)
	c = stupid_rgb_color(name);
    return c;
}
